...
static void update_user_location(sslog_node_t* node, sslog_individual_t* user_individual, double lat, double lon) {
    sslog_individual_t* new_location_individual = sslog_new_individual(CLASS_POINT, rand_uuid("user_point"));
    sslog_insert_property(new_location_individual, PROPERTY_LATITUDE, double_to_string(lat));
    sslog_insert_property(new_location_individual, PROPERTY_LONGITUDE, double_to_string(lon));
...
static sslog_individual_t* publish_search_request(sslog_node_t* node, sslog_individual_t* user_individual) {
    sslog_individual_t* request_individual = sslog_new_individual(CLASS_SEARCHREQUEST, rand_uuid("search_request"));
    sslog_insert_property(request_individual, PROPERTY_CREATEDBY, user_individual);
...
static bool process_subscription_result(sslog_node_t* node, sslog_individual_t* request_individual) {
    ...
    list_for_each(iter, &inserted_individuals->links) {
        ...
        double lat = parse_double(sslog_get_property(point_individual, PROPERTY_LATITUDE));
        double lon = parse_double(sslog_get_property(point_individual, PROPERTY_LONGITUDE));

        printf("%d: Found point %lf %lf\n", counter, lat, lon);

        sslog_individual_t* point2 = sslog_new_individual(CLASS_POINT, rand_uuid("route_point"));
        sslog_insert_property(point2, PROPERTY_LATITUDE, double_to_string(lat));
        sslog_insert_property(point2, PROPERTY_LONGITUDE, double_to_string(lon));
...
static void subscribe_response(sslog_node_t* node, sslog_individual_t* request_individual) {
    ...
    list_add_data(properties, PROPERTY_PROCESSEDAT);
...
int main() {
    ...
    update_user_location(node, user_individual, 61.78, 34.35);

    sslog_individual_t* request_individual = publish_search_request(node, user_individual);
