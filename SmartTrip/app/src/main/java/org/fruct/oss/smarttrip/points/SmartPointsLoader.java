...
public class SmartPointsLoader implements PointsLoader {
	...
	@Override
	public List<Point> loadPoints(final double latCenter, final double lonCenter,
								  final double radius, @Nullable final String pattern) {
		...
		handler.post(new Runnable() {
			@Override
			public void run() {
				startLoadPoints(latCenter, lonCenter, radius, pattern);
			}
		});
	...
	private void startLoadPoints(double latCenter, double lonCenter, double radius, @Nullable  String pattern) {
		...
		if (pattern != null) {
			intent.putExtra(SmartService.EXTRA_PATTERN, pattern);
		}
