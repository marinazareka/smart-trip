...
public class OpenStateTransition implements StateTransition {
	private Random random;

	public OpenStateTransition(Random random) {
		this.random = random;
	}
	...
	public State transition(State state) {
		...
		if (state.getPath().length < 3) {
			return state;
		}

		int i1, i2;
		do {
			i1 = random.nextInt(path.length - 1) + 1;
			i2 = random.nextInt(path.length - 1) + 1;
		} while (i1 == i2);
