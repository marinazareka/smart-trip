...
static const char* URL_FORMAT = "%s/?key=%s&function=place.search&q=&lat=%lf&lon=%lf&format=json&data_blocks=main%%2Clocation%%2C"
                                "&page=1&count=50&category=school&distance=%d";
...
static char* get_points_as_json(double lat, double lon, double radius) {
    char* ret;
    ...
    // Build URL
    int bytes = asprintf(&url, URL_FORMAT, URL_PREFIX, wm_key, lat, lon, (int) radius);
    if (bytes < 0) {
...
static void load_points(double lat, double lon, double radius, struct Point** out_points, int* out_point_count) {
    char* points_json = get_points_as_json(lat, lon, radius);
    if (points_json == NULL) {
